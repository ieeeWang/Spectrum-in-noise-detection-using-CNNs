# -*- coding: utf-8 -*-
"""
Created on Mon Jun  8 15:30:59 2020
my own defined loss (using np) trigger error: No gradients provided for any variable
@author: lwang
"""
...
def loss(model, x, y):
    ...
    y = y.reshape(len(y),4,3)
    yx = y[:,:,0] # x loc (8,4)
    yy = y[:,:,1:3] # (8,2,3)
        
    y_ = y_.reshape(len(y_),4,3)
    y_x = y_[:,:,0] # x loc   
    y_y = y_[:,:,1:3] # y loc
 
    # x_loss = tf.keras.losses.mean_squared_error(yx, y_x)
    x_mse = np.sqrt((yx-y_x)**2) # x loc (8,4)
    
    # IOU in [0, 1]
    IOU_batch = np.ones([len(yy), 4]) # (8,4)
    for i in range(len(yy)):
        tmp_y = yy[i]# (4,2)
        tmp_y_ = y_y[i] # (4,2)
        for j in range(len(tmp_y)):
            A = (tmp_y[j,0], tmp_y[j,1])
            B = (tmp_y_[j,0], tmp_y_[j,1])
            IOU_batch[i,j] = get_IOU(A, B)
    
    # x_loss_mean = tf.reduce_mean(batch_loss/)
    epsilon=1e-07        
    loss = x_mse/(IOU_batch+epsilon)
    loss_batch = np.sum(loss)
    loss_batch = tf.cast(loss_batch, tf.float32)
    return loss_batch
...
for i in range(num):
    ...
    ax.set_xlabel('p:{}'.format(labels_[i,:]))
plt.tight_layout()
plt.show()
